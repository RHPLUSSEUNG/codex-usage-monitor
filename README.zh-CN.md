...
设置窗口保持打开时，所有外观更改都会立即预览到 Compact Bar。三个预设槽位可保存和恢复样式、背景、显示基准、指标开关、显示方式及颜色。使用**保存槽位**记录当前外观，使用**加载**应用预设。

设置分为**常规**、**外观**和**指标**选项卡，三个预设槽位始终显示在所有选项卡上方。选择样式时会同时应用相匹配的默认背景色和透明度，之后仍可自行调整。圆形仪表样式的高度会自动匹配当前显示器的任务栏高度。
