...
설치본은 시작할 때와 6시간마다 GitHub의 정식 릴리스를 확인합니다. 새 버전이 있으면 Windows 알림과 트레이 메뉴의 **v…(으)로 업데이트** 항목이 표시됩니다. 이 항목을 선택하면 릴리스 파일을 다운로드하고 SHA-256 체크섬을 검증한 뒤 설치본을 교체하고 자동으로 다시 실행합니다. 설정 파일은 유지됩니다.
