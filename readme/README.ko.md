...
설치본은 시작할 때와 6시간마다 GitHub의 정식 릴리스를 확인합니다. 새 버전이 있으면 Windows 알림과 트레이 메뉴의 **v…(으)로 업데이트** 항목이 표시됩니다. 이 항목을 선택하면 릴리스 파일을 다운로드하고 SHA-256 체크섬을 검증한 뒤 설치본을 교체하고 자동으로 다시 실행합니다. 체크섬은 다운로드 무결성을 확인하지만 게시자 서명을 대신하지는 않습니다. 설정 파일은 유지됩니다.

언제든 트레이 메뉴의 **업데이트 확인**을 선택해 직접 확인할 수도 있습니다. 자동 설치는 위에 표시된 설치 경로에서 실행한 경우에만 동작하며 소스·디버그 빌드는 덮어쓰지 않습니다.
...
설치본을 제거하려면 트레이 메뉴에서 **제거**를 선택합니다. 확인창에서 설정, 프리셋 및 로그를 유지하거나 함께 삭제할 수 있습니다. 제거 시 시작 메뉴 바로가기와 Windows 자동 실행 등록도 삭제됩니다.
...
- 트레이 아이콘 오른쪽 클릭: 화면 밖 위치 초기화 또는 진단 정보 복사
...
트레이 메뉴에서 **진단 정보 복사**를 선택합니다. 앱 버전, app-server 상태, 마지막 갱신·오류, Windows/DPI/모니터 정보와 설정·로그 경로가 클립보드에 복사됩니다.
