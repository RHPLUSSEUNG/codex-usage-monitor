...
インストール版は起動時と 6 時間ごとに GitHub の正式リリースを確認します。新しいバージョンがある場合は、Windows 通知とトレイメニューの **v… に更新** が表示されます。選択するとリリースファイルをダウンロードし、SHA-256 チェックサムを検証してからインストール済みアプリを置き換え、自動的に再起動します。設定ファイルは維持されます。
